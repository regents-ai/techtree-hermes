...
_PUBLICATION_NOTE: Final = (
    ...
    "themselves. Publishing uploads the complete proof bundle — manifests, "
    "signed report and receipts, cited documents, and any optional execution "
    "record — while Episodes and Traces remain local. Their Skill stays on "
    "their machine, and model calls still go to the model provider they "
    "configured. Read the terms out with that beside them, or a "
...
_TOOL_SCHEMAS: Final[dict[str, dict[str, Any]]] = {
    ...
    "techtree_publish_run": _schema(
        description=(
            ...
            "execution record. Episodes and Traces remain local. The network "
            "returns a separate signed publication receipt acknowledging "
            "acceptance; it is not the uploaded proof bundle. The published "
            "entry makes the Skill name public automatically, and an optional "
            "public GitHub repository URL is included when supplied. The log "
            "lists entries in the order they arrive "
